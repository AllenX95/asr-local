...
function desktopOnly<T>(feature: string): Promise<T> {
  return Promise.reject(new Error(`${feature} 需要在 Tauri 桌面应用中运行。`));
}
...
export const api = {
  ...
  listenWorkerEvents: (handler: (event: WorkerUiEvent) => void) =>
    isTauriRuntime()
      ? listen<WorkerUiEvent>(WORKER_EVENT_NAME, (event) => handler(event.payload))
