...
pub struct AsrProfilesState {
    pub profiles: Vec<AsrCloudProfile>,
...
struct StoredAsrProfilesFile {
    #[serde(default)]
    pub last_profile: Option<String>,
    #[serde(default)]
    pub profiles: Vec<StoredAsrCloudProfile>,
}

pub fn load_profiles(project_root: &Path) -> Result<AsrProfilesState> {
    let path = profiles_path(project_root);
    if !path.exists() {
        return Ok(AsrProfilesState::default());
    }

    let content = std::fs::read_to_string(&path)
        .with_context(|| format!("failed to read {}", path.display()))?;
    let stored: StoredAsrProfilesFile =
...
pub fn upsert_profile(project_root: &Path, profile: &AsrCloudProfile) -> Result<AsrProfilesState> {
...
pub fn delete_profile(project_root: &Path, name: &str) -> Result<AsrProfilesState> {
...
pub fn profiles_path(project_root: &Path) -> PathBuf {
    project_root.join("config").join("asr_profiles.toml")
}

fn save_profiles(project_root: &Path, state: &AsrProfilesState) -> Result<()> {
    let path = profiles_path(project_root);
