...
class PyannoteDiarizationProvider:
    def __init__(self, *, model_manager: Any, execution_gate=None) -> None:
    ...
    def diarize(
        ...
        *,
        ...
        # Loading, device movement performed by the manager, and inference
        # must not overlap the shared Qwen CUDA call.  The gate is intentionally
        # released before returning to the caller; no Future is awaited here.
    ...
    def close(self) -> None:
        ...
        if callable(close):
            # ModelManager already owns the same gate when it was constructed
            # by ChunkedLocalTranscriber. Avoid requiring custom test gates to
            # be re-entrant while still covering managers without an internal
            # gate here.
            if getattr(self.model_manager, "execution_gate", None) is self.execution_gate:
                close()
            else:
                with self._gate_context():
                    close()
