...
$stagingDir = Join-Path $desktopDir 'release-electron-staging'
...
try {
    ...
    if ((Test-Path -LiteralPath $backupAppDir) -and -not (Test-Path -LiteralPath $targetAppDir)) {
        Move-Item -LiteralPath $backupAppDir -Destination $targetAppDir
    } else {
        Remove-SafeDirectory $backupAppDir
    }
    $hadPreviousPackage = Test-Path -LiteralPath $targetAppDir
    if ($hadPreviousPackage) {
        Move-Item -LiteralPath $targetAppDir -Destination $backupAppDir
    }
    try {
        Move-Item -LiteralPath $stagedAppDir -Destination $targetAppDir
    } catch {
